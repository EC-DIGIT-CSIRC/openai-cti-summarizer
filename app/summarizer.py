...
def _build_openai_model(settings: LLMSettings) -> Any:
...
def _build_azure_model(settings: LLMSettings) -> Any:
    ...
    model_class = OpenAIResponsesModel if _is_gpt_5_5_model(settings.model) else OpenAIChatModel
    return model_class(settings.model, provider=provider)


def _build_openrouter_model(settings: LLMSettings) -> Any:
...
def _build_ollama_model(settings: LLMSettings) -> Any:
...
def _build_anthropic_model(settings: LLMSettings) -> Any:
    if not settings.api_key:
...
def _build_pydantic_ai_model(settings: LLMSettings) -> Any:
    ...
    builders = {
        LLMProvider.OPENAI: _build_openai_model,
        LLMProvider.AZURE: _build_azure_model,
        LLMProvider.OPENROUTER: _build_openrouter_model,
        LLMProvider.OLLAMA: _build_ollama_model,
        LLMProvider.ANTHROPIC: _build_anthropic_model,
    }
